...
function fromArrayBuffer( buf, byteOffset, length ) {
	...
	if ( !isArrayBuffer( buf ) ) {
		throw new TypeError( format( '0GL2B,F0', buf ) );
	}
	if ( arguments.length > 1 ) {
		if ( !isNonNegativeInteger( byteOffset ) ) {
			throw new TypeError( format( '0GL3X,F1', byteOffset ) );
		}
		if ( byteOffset > buf.byteLength ) {
			throw new RangeError( format( '0GL3Y,F2', byteOffset ) );
		}
		offset = byteOffset;
		if ( arguments.length > 2 ) {
			if ( !isNonNegativeInteger( length ) ) {
				throw new TypeError( format( '0GL3Z,F3', length ) );
			}
			if ( length > buf.byteLength ) {
				throw new RangeError( format( '0GL3a,F4', length ) );
			}
			len = length;
		} else {
